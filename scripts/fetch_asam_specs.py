...
"""Fetch ASAM specifications as local Markdown references for Kinema.
...
USER_AGENT = "Kinema-spec-fetch (github.com/Robomous/Kinema)"
REQUEST_DELAY_S = 0.5
...
# Topic -> chapter-directory keyword lookup, tuned to Kinema work.
...
def file_header(spec_name: str, version: str, source_url: str) -> str:
    ...
    return (
        f"<!--\nsource: {source_url}\nspec: {spec_name}\nversion: {version}\n"
        f"fetched: {today}\n"
        "© ASAM e.V. — fetched for local reference use in Kinema development.\n"
...
def write_master_index(out_root: Path) -> None:
    ...
    lines = [
        ...
        "",
        "Local text copies of the ASAM standards Kinema implements.",
