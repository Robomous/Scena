...
|---|---|---|---|---|---|
| SpeedAction | §7.4.1.4 | In | p2-s2, p5-s4, p4-s2 | `longitudinal_test.cpp`, `action_longitudinal_test.cpp`, `test_longitudinal.py`, `xml_storyboard_test.cpp` | Kernel landed: all TransitionDynamics shapes (linear/cubic/sinusoidal/step) × dimensions (time/rate/distance), default longitudinal controller, position-mode + hard Performance clamp (max speed + per-shape peak accel) (p2-s2). Relative target (§RelativeTargetSpeed): delta/factor, one-shot + continuous tracking that never ends (§7.5.3); minimal single-domain supersession (p5-s4, ADR-0013). `followingMode=follow` jerk deferred (ADR-0011, #62). XML lowering landed (p4-s2, `xml_storyboard_test.cpp`) |
| SpeedProfileAction | §7.4.1.4 | In | p2-s2, p4-s2 | `longitudinal_test.cpp`, `test_longitudinal.py`, `xml_storyboard_test.cpp` | Kernel landed (≥1.2): position-mode piecewise-linear entry series, omitted entry time ⇒ performance-limited. entityRef-relative profile + DynamicConstraints/jerk (`followingMode=follow`) deferred (ADR-0011, #62). XML lowering landed (p4-s2, `xml_storyboard_test.cpp`) |
| LongitudinalDistanceAction | §7.4.1.4 | In | p5-s5, p4-s2 | `action_distance_test.cpp`, `test_distance_keeping.py`, `xml_storyboard_test.cpp` | Kernel landed: distance and timeGap targets, freespace (OBB) and reference-point gaps, `displacement` (any/leading/trailing), DynamicConstraints + Performance clamping, `continuous` keeping that never ends (§7.5.3); longitudinal-domain owner, so it supersedes and is superseded by SpeedAction/SpeedProfileAction (ADR-0014). Road-based coordinateSystem → p3-s4 (UnsupportedFeature warning + immediate completion); DynamicConstraints jerk rates stored but not clamped (#62). XML lowering landed (p4-s2, `xml_storyboard_test.cpp`) |
| LaneChangeAction | §7.4.1.4 | In | p2-s3, p4-s2 | `action_lateral_test.cpp`, `lateral_test.cpp`, `determinism_test.cpp`, `test_lateral.py`, `xml_storyboard_test.cpp` | Kernel landed: relative and absolute target lane, `targetLaneOffset`, all TransitionDynamics shapes × dimensions (Distance tied to the odometer, Step instantaneous), heading-blended lateral kinematics, lateral-domain supersession (ADR-0016). Flat-world lane widths (configurable `default_lane_width`) with a forward-pulled IRoadQuery lane-query path; absolute-lane/road-backed resolution remains with #23 (UnsupportedFeature warning + completion). 1.4 lane-layer awareness excluded (Scena targets 1.0–1.3). XML lowering landed (p4-s2, `xml_storyboard_test.cpp`) |
| LaneOffsetAction | §7.4.1.4 | In | p2-s3, p4-s2 | `action_lateral_test.cpp`, `lateral_test.cpp`, `test_lateral.py`, `xml_storyboard_test.cpp` | Kernel landed: absolute and relative offset targets, duration derived from `maxLateralAcc` per shape (k = 4 / π²/2 / 6; missing ⇒ 'inf' ⇒ instantaneous, as is Step), `continuous` keeping that never ends (§7.5.3) and re-corrects when the target moves (ADR-0016). A zero `maxLateralAcc` is rejected at init. XML lowering landed (p4-s2, `xml_storyboard_test.cpp`) |
| LateralDistanceAction | §7.4.1.4 | In | p2-s3, p4-s2 | `action_lateral_test.cpp`, `test_lateral.py`, `xml_storyboard_test.cpp` | Kernel landed: freespace (OBB) and reference-point lateral gaps, `displacement` (any/left/right), DynamicConstraints-limited or rigid keeping, `continuous` that never ends (§7.5.3); lateral-domain owner (ADR-0016). Lane coordinateSystem is undefined by §6.4.8.2.2 (warned at init + completion); road/trajectory coordinateSystem → p3-s4. Performance is deliberately not applied (its limits are longitudinal). XML lowering landed (p4-s2, `xml_storyboard_test.cpp`) |
| TeleportAction | §7.4.1.4 | In | p5-s4, p2-s4, p4-s2 | `action_teleport_test.cpp`, `control_ownership_test.cpp`, `test_private_actions.py`, `xml_storyboard_test.cpp` | Kernel landed: step (instantaneous) write of the entity's full pose (position + orientation) from any §6.3.8 Position resolved through the `PositionResolver`; self-contained variants resolve, road/route/geo/trajectory reported (p3-s4/p2-s5). A teleport on a host-controlled entity is a reported mode violation (ADR-0013, ADR-0017). XML lowering landed (p4-s2, `xml_storyboard_test.cpp`) |
...
| VisibilityAction | §7.4.1.4 | In | p5-s5, p4-s2 | `action_controller_visibility_test.cpp`, `xml_storyboard_test.cpp` | Kernel landed: graphics/sensors/traffic flags stored per entity, readable via `Engine::visibility_of` and handed to the host through `ISimulatorGateway::on_visibility_changed`; no sensor semantics in the engine (ADR-0014). `sensorReferenceSet` (1.2+) → Post. XML lowering landed (p4-s2, `xml_storyboard_test.cpp`) |
| ControllerAction (wrapper) | §7.4.1.4, §6.6 | In | p5-s5 | `action_controller_visibility_test.cpp` | Wrapper for the three below |
| AssignControllerAction | §6.6.3 | In | p5-s5, p4-s2 | `action_controller_visibility_test.cpp`, `xml_storyboard_test.cpp` | Kernel landed: controller name, controllerType (1.3) and ordered properties stored per entity and handed to the host through `ISimulatorGateway::on_controller_assigned`; activateLateral/activateLongitudinal applied, activation outside the controllerType rejected (rule `scenario_logic.controller_activation`). Lighting/animation activation → Post (appearance). XML lowering landed (p4-s2, `xml_storyboard_test.cpp`) |
| ActivateControllerAction | §6.6.4 | In | p5-s5, p4-s2 | `action_controller_visibility_test.cpp`, `xml_storyboard_test.cpp` | Kernel landed: the lateral/longitudinal flags toggle Scena's engine default controller, deactivation retiring the domain's current owner and suppressing later actions on it (ADR-0014); lighting/animation domains → Post (appearance). Deprecated direct-under-PrivateAction placement accepted (a frontend concern, P4). XML lowering landed (p4-s2, `xml_storyboard_test.cpp`) |
...
| AssignRouteAction | §6.8.2 | In | p5-s5, p4-s2 | `action_routing_test.cpp`, `test_routing_actions.py`, `xml_storyboard_test.cpp` | Assignment landed: Route/Waypoint/RouteStrategy over world-frame waypoints, installed per entity and readable via `Engine::route_of`, overwriting any prior route (§6.8.2) and completing immediately (Table 10) without touching a control domain (§7.4.1.4). Road semantics — waypoint-on-road prerequisites, RouteStrategy interpretation, route-following motion — → p3-s4. XML lowering landed (p4-s2, `xml_storyboard_test.cpp`) |
| FollowTrajectoryAction | §6.9 | In | p2-s5, p5-s5, p4-s2 | `action_routing_test.cpp`, `trajectory_test.cpp`, `determinism_test.cpp`, `test_trajectory.py`, `capi_test.cpp`, `xml_storyboard_test.cpp` | Polyline (p5-s5) plus Clothoid and NURBS shapes (p2-s5, ADR-0018): both drive through the arc-length evaluator, `timeReference` none (entity's own speed) and timing (linear time→arclength over the shape's start/end times), `initialDistanceOffset`, exact completion at the trajectory end. `followingMode=follow` accepted-as-position and closed trajectories accepted-as-open, both with an UnsupportedFeature warning (true `follow` steering deferred). ClothoidSpline, the Motion element and trajectory catalogs remain out (see below). XML lowering landed (p4-s2, `xml_storyboard_test.cpp`) |
| AcquirePositionAction | §7.4.1.4 | In | p5-s5, p2-s4, p4-s2 | `action_routing_test.cpp`, `xml_storyboard_test.cpp` | Assignment landed: the implicit two-waypoint route of §7.4.1.4 (position at apply time → target, strategy shortest), installed and completed immediately (Table 10). The target is any §6.3.8 Position resolved through the `PositionResolver` (p2-s4, ADR-0017); road semantics → p3-s4. XML lowering landed (p4-s2, `xml_storyboard_test.cpp`) |
...
| Element | Section | Status | Sprint | Tests | Notes |
|---|---|---|---|---|---|
| EnvironmentAction | §7.4.2 | In | p5-s6, p4-s2 | `action_global_test.cpp`, `test_global_actions.py`, `xml_storyboard_test.cpp` | Kernel landed: Environment/Weather/Sun/Fog/Precipitation/Wind/RoadCondition value types merged member-wise per §Environment ("a missing condition doesn't change"), readable via `Engine::environment`; TimeOfDay re-anchors the simulated clock and its `animation` flag freezes or advances it, feeding TimeOfDayCondition unchanged (ADR-0015). No physics or visual coupling — weather is stored, never applied (documented simplification). `RoadCondition.wetness` (1.2) + Properties, the DomeImage, and the CatalogReference form → Post/P4. XML lowering landed (p4-s2, `xml_storyboard_test.cpp`) |
| AddEntityAction | §7.4.2 | In | p5-s6, p2-s4, p4-s2 | `action_global_test.cpp`, `xml_storyboard_test.cpp` | Kernel landed: an `active` flag on the entity record (never an erase/insert, so iteration order and bookkeeping are untouched); a fresh state at any §6.3.8 Position resolved through the `PositionResolver` — full pose incl. orientation — with the observation baseline seeded as at init (p2-s4, ADR-0017). Adding an active entity is a no-op (§7.4.2). XML lowering landed (p4-s2, `xml_storyboard_test.cpp`) |
| DeleteEntityAction | §7.4.2 | In | p5-s6, p4-s2 | `action_global_test.cpp`, `xml_storyboard_test.cpp` | Kernel landed: clears all runtime motion/assignment/observation state, keeps the declared immutables; the entity is skipped by poll/refresh/integrate/publish, reports nothing to the host, and is absent from the by-entity conditions. Running private actions on it or on it as a reference stop (§7.5.2.2), surfacing as the owning event ending one evaluation later (ADR-0015). Deleting an inactive entity is a no-op. XML lowering landed (p4-s2, `xml_storyboard_test.cpp`) |
| ParameterSetAction | class ref | In | p5-s6, p4-s2 | `action_global_test.cpp`, `xml_storyboard_test.cpp` | Kernel landed: accepted, deprecated with 1.2, executed against a runtime overlay consulted ahead of the immutable §9.1 declaration so a 1.0/1.1 file's ParameterCondition observes it; warn-once `DeprecatedFeature` (ADR-0015). XML lowering landed (p4-s2, `xml_storyboard_test.cpp`) |
| ParameterModifyAction (+ ModifyRule, Add/MultiplyByValueRule) | class ref | In | p5-s6, p4-s2 | `action_global_test.cpp`, `xml_storyboard_test.cpp` | Kernel landed: same overlay and deprecation warning; add/multiplyByValue over the overlay, numeric values only per rule C.2.6. XML lowering landed (p4-s2, `xml_storyboard_test.cpp`) |
| VariableSetAction | §6.12 | In | p5-s6, p4-s2 | `action_global_test.cpp`, `xml_storyboard_test.cpp` | Kernel landed (≥1.2): writes the runtime variable store a VariableCondition and the host both read. Undeclared ref rejected at init (rule reference_control.resolvable_variable_reference). XML lowering landed (p4-s2, `xml_storyboard_test.cpp`) |
| VariableModifyAction (+ rules) | §6.12 | In | p5-s6, p4-s2 | `action_global_test.cpp`, `xml_storyboard_test.cpp` | Kernel landed (≥1.2): add/multiplyByValue as one fixed IEEE expression, stored back through the shortest round-tripping decimal so a modify chain is exact. Non-numeric current value ⇒ warn-once citing rule data_type.variable_modification_or_comparison_possible (C.2.6) and no-op; typed declarations → p4-s3. XML lowering landed (p4-s2, `xml_storyboard_test.cpp`) |
| TrafficSignalControllerAction | §6.11 | In | p5-s6, p4-s2 | `traffic_signal_test.cpp`, `test_traffic_signals.py`, `xml_storyboard_test.cpp` | Kernel landed: restarts a controller's cycle at a named phase (re-anchor + immediate tick, so same-evaluation conditions agree) and continues in declared order; both references validated at init (rule reference_control.traffic_signal_controller_action_references, C.7.11). 1.4 phase *semantics* (TrafficSignalSemantics) excluded. XML lowering landed (p4-s2, `xml_storyboard_test.cpp`) |
| TrafficSignalStateAction | §6.11 | In | p5-s6, p4-s2 | `traffic_signal_test.cpp`, `xml_storyboard_test.cpp` | Kernel landed: forces a named signal's observable state, which stands until the controlling cycle's next phase transition (actions-win precedence, the §11.12 bulb-failure shape, ADR-0015). Signal id free-form — rule C.7.14 needs a road network (p3-s4). XML lowering landed (p4-s2, `xml_storyboard_test.cpp`) |
...
| Element | Section | Status | Sprint | Tests | Notes |
|---|---|---|---|---|---|
| UserDefinedAction / CustomCommandAction | §7.4.3 | In | p5-s6, p4-s2 | `action_global_test.cpp`, `test_global_actions.py`, `xml_storyboard_test.cpp` | Kernel landed: `type` and `content` handed verbatim to `ISimulatorGateway::on_custom_command` (defaulted virtual, an ADR-0003 amendment); a silent no-op without a gateway, per §7.4.3 executability — no diagnostic (ADR-0015). XML lowering landed (p4-s2, `xml_storyboard_test.cpp`) |
...
| Element | Section | Status | Sprint | Tests | Notes |
|---|---|---|---|---|---|
| AccelerationCondition | §7.6.5.1 | In | p5-s2, p4-s2 | `condition_byentity_kinematics_test.cpp`, `test_byentity_conditions.py`, `xml_storyboard_test.cpp` | Kernel landed: finite-difference acceleration, absent (⇒false) until two samples; optional direction (longitudinal signed, lateral/vertical 0 in the scalar model). XML lowering landed (p4-s2, `xml_storyboard_test.cpp`) |
| AngleCondition | §7.6.5.1 | Post | — | — | Introduction version unconfirmed in local text *(verify)*; low demand for v0.0.1 set |
| CollisionCondition | §7.6.5.1 | In | p5-s3, p4-s2 | `condition_byentity_interaction_test.cpp`, `test_interaction_conditions.py`, `xml_storyboard_test.cpp` | Kernel landed: bounding-box (2D OBB) intersection, touching ⇒ collision (§6.4.7.2); absent box on either entity ⇒ false. EntityRef target only — ByObjectType categories landed p2-s1 (ADR-0010); wiring the target into this condition is a follow-up. XML lowering landed (p4-s2, `xml_storyboard_test.cpp`) |
| DistanceCondition | §7.6.5.1, §6.4 | In | p5-s3, p4-s2 | `condition_byentity_interaction_test.cpp`, `test_interaction_conditions.py`, `xml_storyboard_test.cpp` | Kernel landed: euclidean (3D reference-point / 2D freespace) and longitudinal/lateral (entity or world axis); road/lane/trajectory CS ⇒ deterministic false + UnsupportedFeature warning (deferred p3-s4). Deprecated `alongRoute`/`cartesianDistance` warned and mapped. XML lowering landed (p4-s2, `xml_storyboard_test.cpp`) |
| EndOfRoadCondition | §7.6.5.1 | In | p5-s3, p4-s2 | `condition_byentity_interaction_test.cpp`, `test_interaction_conditions.py`, `xml_storyboard_test.cpp` | IR + validation landed: requires road-network topology (p3-s4), so it evaluates deterministic false with a rule-cited UnsupportedFeature warning until then. XML lowering landed (p4-s2, `xml_storyboard_test.cpp`) |
| OffroadCondition | §7.6.5.1 | In | p5-s3, p4-s2 | `condition_byentity_interaction_test.cpp`, `test_interaction_conditions.py`, `xml_storyboard_test.cpp` | IR + validation landed: road-deferred like EndOfRoad — deterministic false + UnsupportedFeature warning until p3-s4. XML lowering landed (p4-s2, `xml_storyboard_test.cpp`) |
| ReachPositionCondition | §7.6.5.1 | In | p5-s2, p4-s2 | `condition_byentity_kinematics_test.cpp`, `test_byentity_conditions.py`, `xml_storyboard_test.cpp` | Kernel landed: deprecated (1.2) ⇒ DeprecatedFeature warning, still evaluated; 2D horizontal tolerance circle (z ignored) against a minimal WorldPosition. The `PositionResolver` landed in p2-s4 (ADR-0017) but was wired into the position **actions** only; generalizing the condition's target to any §6.3.8 variant follows when road targets are needed (p3-s4). XML lowering landed (p4-s2, `xml_storyboard_test.cpp`) |
| RelativeAngleCondition | §7.6.5.1 | Post | — | — | Same rationale as AngleCondition *(verify)* |
| RelativeClearanceCondition | §7.6.5.1 | In | p5-s3, p4-s2 | `condition_byentity_interaction_test.cpp`, `test_interaction_conditions.py`, `xml_storyboard_test.cpp` | IR + validation landed (freeSpace/oppositeLanes, distance window, entity refs, RelativeLaneRange): the checked area is in lane coordinates (§6.4.5), so it evaluates deterministic false + UnsupportedFeature warning until p3-s4. XML lowering landed (p4-s2, `xml_storyboard_test.cpp`) |
| RelativeDistanceCondition | §7.6.5.1, §6.4 | In | p5-s3, p4-s2 | `condition_byentity_interaction_test.cpp`, `test_interaction_conditions.py`, `xml_storyboard_test.cpp` | Kernel landed: entity-to-entity distance, relativeDistanceType required, no alongRoute; freespace via bounding boxes; road/lane/trajectory CS ⇒ deterministic false + UnsupportedFeature warning (p3-s4). XML lowering landed (p4-s2, `xml_storyboard_test.cpp`) |
| RelativeSpeedCondition | §7.6.5.1 | In | p5-s2, p4-s2 | `condition_byentity_kinematics_test.cpp`, `test_byentity_conditions.py`, `xml_storyboard_test.cpp` | Kernel landed: spec formula `speed_rel = speed(trig) − speed(ref)` (signed); directional projection in the triggering frame via det_sincos; absent reference ⇒ false. XML lowering landed (p4-s2, `xml_storyboard_test.cpp`) |
| SpeedCondition | §7.6.5.1 | In | p5-s2, p4-s2 | `condition_byentity_kinematics_test.cpp`, `test_byentity_conditions.py`, `xml_storyboard_test.cpp` | Kernel landed: total = \ | speed\ | , optional directional projection. XML lowering landed (p4-s2, `xml_storyboard_test.cpp`) |
| StandStillCondition | §7.6.5.1 | In | p5-s2, p4-s2 | `condition_byentity_kinematics_test.cpp`, `test_byentity_conditions.py`, `xml_storyboard_test.cpp` | Kernel landed: contiguous time at speed exactly 0.0 (no invented ε), `>=` duration. XML lowering landed (p4-s2, `xml_storyboard_test.cpp`) |
| TimeHeadwayCondition | §7.6.5.1, §6.4 | In | p5-s3, p4-s2 | `condition_byentity_interaction_test.cpp`, `test_interaction_conditions.py`, `xml_storyboard_test.cpp` | Kernel landed: distance ÷ the triggering entity's speed only (reference leading); stopped/reversing follower ⇒ false. Full distance matrix incl. freespace; deprecated `alongRoute` warned and mapped; road CS deferred false (p3-s4). XML lowering landed (p4-s2, `xml_storyboard_test.cpp`) |
| TimeToCollisionCondition | §7.6.5.1, §6.4 | In | p5-s3, p4-s2 | `condition_byentity_interaction_test.cpp`, `test_interaction_conditions.py`, `xml_storyboard_test.cpp` | Kernel landed: distance ÷ closing speed (no acceleration), entity XOR position target; diverging/zero closing speed or coincident points ⇒ false. Road CS deferred false (p3-s4). XML lowering landed (p4-s2, `xml_storyboard_test.cpp`) |
| TraveledDistanceCondition | §7.6.5.1 | In | p5-s2, p4-s2 | `condition_byentity_kinematics_test.cpp`, `test_byentity_conditions.py`, `xml_storyboard_test.cpp` | Kernel landed: cumulative world-frame path length from init (not displacement), `>=` value. XML lowering landed (p4-s2, `xml_storyboard_test.cpp`) |

## Conditions — by value

| Element | Section | Status | Sprint | Tests | Notes |
|---|---|---|---|---|---|
| ParameterCondition | §7.6.5.2 | In | p5-s1, p4-s2 | `condition_byvalue_test.cpp`, `test_byvalue_conditions.py`, `xml_storyboard_test.cpp`, `xml_byvalue_test.cpp` | Kernel landed: shared `Rule` comparator, exact IEEE numeric compare, ordering only when both operands are scalar-convertible; immutable at runtime (§9.1); dangling `parameterRef` ⇒ init `SemanticError`. XML lowering landed (p4-s2, `xml_storyboard_test.cpp`, `xml_byvalue_test.cpp`). XML lowering proven per member (p5-s1 follow-up #57): the `rule` literal set with a version gate that warns rather than rejects (introduction versions unverifiable from the local text, OQ-4) |
| VariableCondition | §7.6.5.2, §6.12 | In | p5-s1, p4-s2 | `condition_byvalue_test.cpp`, `test_byvalue_conditions.py`, `xml_storyboard_test.cpp`, `xml_byvalue_test.cpp` | Kernel landed (≥1.2): runtime variable store seeded at init, host `set_variable`; strings equal/not-equal, numeric when scalar-convertible; dangling `variableRef` ⇒ init `SemanticError` (rule `reference_control.resolvable_variable_reference`). XML lowering landed (p4-s2, `xml_storyboard_test.cpp`, `xml_byvalue_test.cpp`). XML lowering proven per member (p5-s1 follow-up #57): the `rule` literal set with a version gate that warns rather than rejects (introduction versions unverifiable from the local text, OQ-4) |
| SimulationTimeCondition | §7.6.5.2 | In | p5-s1, p4-s2 | `condition_byvalue_test.cpp`, `test_byvalue_conditions.py`, `xml_storyboard_test.cpp`, `xml_byvalue_test.cpp` | Kernel landed: gained the spec's `rule` (default greaterOrEqual); time starts when the Storyboard enters running (§8.4.7); NaN value ⇒ init `ValidationError`. XML lowering landed (p4-s2, `xml_storyboard_test.cpp`, `xml_byvalue_test.cpp`). XML lowering proven per member (p5-s1 follow-up #57): the `rule` literal set with a version gate that warns rather than rejects (introduction versions unverifiable from the local text, OQ-4) |
| StoryboardElementStateCondition | §7.6.5.2, §8.1–8.2 | In | p5-s1, p4-s2 | `condition_byvalue_test.cpp`, `test_byvalue_conditions.py`, `xml_storyboard_test.cpp`, `xml_byvalue_test.cpp` | Kernel landed: level states + one-evaluation transition pulses; nameRef `::` resolution; zero/ambiguous ref ⇒ init `SemanticError` (rule `reference_control.resolvable_storyboard_element_ref`); `action` type unsupported until p5-s4 (warns). XML lowering landed (p4-s2, `xml_storyboard_test.cpp`, `xml_byvalue_test.cpp`). XML lowering proven per member (p5-s1 follow-up #57): the `rule` literal set with a version gate that warns rather than rejects (introduction versions unverifiable from the local text, OQ-4) |
| TimeOfDayCondition | §7.6.5.2 | In | p5-s1, p4-s2 | `condition_byvalue_test.cpp`, `test_byvalue_conditions.py`, `xml_storyboard_test.cpp`, `xml_byvalue_test.cpp` | Kernel landed: host-anchored simulated clock advancing with sim time, epoch-seconds compare (UTC offset honored), leap-year-correct; unset anchor ⇒ false + warn-once. Same anchor fed later by the EnvironmentAction clock (p5-s6). XML lowering landed (p4-s2, `xml_storyboard_test.cpp`, `xml_byvalue_test.cpp`). XML lowering proven per member (p5-s1 follow-up #57): the `rule` literal set with a version gate that warns rather than rejects (introduction versions unverifiable from the local text, OQ-4) |
| TrafficSignalCondition | §7.6.5.2, §6.11 | In | p5-s6, p4-s2 | `traffic_signal_test.cpp`, `test_traffic_signals.py`, `xml_storyboard_test.cpp`, `xml_byvalue_test.cpp` | Kernel landed: a level predicate on the byte-for-byte signal state ("reaches" comes from conditionEdge rising, the p5-s1 catalog precedent); an unwritten signal id is a deterministic false + warn-once. Road-network id validation (rule C.7.10) → p3-s4. XML lowering landed (p4-s2, `xml_storyboard_test.cpp`, `xml_byvalue_test.cpp`). XML lowering proven per member (p5-s1 follow-up #57): the `rule` literal set with a version gate that warns rather than rejects (introduction versions unverifiable from the local text, OQ-4) |
| TrafficSignalControllerCondition | §7.6.5.2, §6.11 | In | p5-s6, p4-s2 | `traffic_signal_test.cpp`, `test_traffic_signals.py`, `xml_storyboard_test.cpp`, `xml_byvalue_test.cpp` | Kernel landed: a level predicate on the controller's phase name; a controller before its §6.11.3 delayed start has no phase, so the condition is a deterministic false. Both references validated at init (rule ...traffic_signal_controller_condition_references, C.7.12). 1.4 semantics excluded. XML lowering landed (p4-s2, `xml_storyboard_test.cpp`, `xml_byvalue_test.cpp`). XML lowering proven per member (p5-s1 follow-up #57): the `rule` literal set with a version gate that warns rather than rejects (introduction versions unverifiable from the local text, OQ-4) |
| UserDefinedValueCondition | §7.6.5.2 | In | p5-s1, p4-s2 | `condition_byvalue_test.cpp`, `test_byvalue_conditions.py`, `xml_storyboard_test.cpp`, `xml_byvalue_test.cpp` | Kernel landed: host-provided named values (`set_user_defined_value`, stageable pre-init); unset name ⇒ false + warn-once. XML lowering landed (p4-s2, `xml_storyboard_test.cpp`, `xml_byvalue_test.cpp`). XML lowering proven per member (p5-s1 follow-up #57): the `rule` literal set with a version gate that warns rather than rejects (introduction versions unverifiable from the local text, OQ-4) |
...
| Init phase (global + private init actions before simulation time) | §8.5 | In | p1-s1, p4-s2 | `storyboard_test.cpp`, `xml_storyboard_test.cpp` | Loading landed (p4-s2): Init/Actions global, user-defined and per-entity private actions lower in document order and are applied concurrently at init. Non-instantaneous init actions carry into the storyboard |
...
| ScenarioDefinition (incl. RoadNetwork reference) | class ref | In | p4-s2 | `xml_storyboard_test.cpp` | Landed (p4-s2, ADR-0021): ParameterDeclarations/VariableDeclarations lower as literal name→value pairs (references and expressions → p4-s3), Entities and Storyboard lower into the IR, CatalogLocations → p4-s4, MonitorDeclarations Post. RoadNetwork logic/scene-graph file paths land on the frontend `Document`, not the IR — the host builds the road backend from them; TrafficSignalControllers declared there lower into the IR |
...
| Entities: ScenarioObject + inline Vehicle/Pedestrian/MiscObject | §7.2.2 | In | p2-s1, p4-s2 | `entity_model_test.cpp`, `entity_types_test.cpp`, `test_entity_model.py`, `xml_storyboard_test.cpp` | IR + bindings landed (p2-s1): ObjectType/category/Role enums (full 1.4 sets, 1.3→1.4 deprecations noted), `Performance`, axles and properties as data, bounding boxes, EntityObject variant + `object_type_of`/`bounding_box_of`/`performance_of`; full h/p/r `EntityState`; C-ABI typed builders + metadata accessors; Performance range validation (ADR-0010). XML lowering landed (p4-s2): ScenarioObject name is the IR id, inline Vehicle/Pedestrian/MiscObject with bounding box, Performance, axles and ordered properties; catalog references, ObjectController and EntitySelection → p4-s4 |
...
| TrafficSignalController / Phase / TrafficSignalState | §6.11 | In | p5-s6, p4-s2 | `traffic_signal_test.cpp`, `test_traffic_signals.py`, `xml_storyboard_test.cpp` | Kernel landed: the ordered cycle with an arithmetic (fmod) phase clock that cannot drift with the host's step pattern, half-open phase intervals, transitive §6.11.3 delay chains, and write-on-transition semantics; validated at init for unique names, non-negative durations (rule C.2.3) and acyclic resolvable references (C.7.13). 1.4 TrafficSignalSemantics / TrafficSignalGroupState excluded. XML lowering landed (p4-s2, `xml_storyboard_test.cpp`) |
