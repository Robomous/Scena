...
| `std` — road abstraction, actions, modifiers, environment, traffic lights | §8.3, §8.5, §8.8–§8.12, §8.15 | In | p7-s5 | Lands with the remaining slices of #43. The road-dependent `physical_object` methods (`road_distance`, `get_route_point`, `distance_along_route`, …) arrive with §8.12 as `extend` blocks, which is how the standard itself prints their prototypes |
